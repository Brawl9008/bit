...
async def mute(message):
   if not message.reply_to_message:
      ...
      return
   keyboard = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("Отсосать хуй за 10к", callback_data=f"unmute:{message.reply_to_message.from_user.id}"))
   try:
...
@dp.query_callback_handler(text_startswith="unmute:")
async def haha(call):
    check_id = int(call.data.split(":")[1])
    if call["from"].id != check_id: return
    for row in db.cursor.execute(f"SELECT money FROM users where id={check_id}"):
        if 10000 > row[0]:
            return await call.answer("нет бабок")
    db.UpdateUserValueMinus("money", 10000, check_id)
    await bot.restrict_chat_member(call.message.chat.id, check_id, types.ChatPermissions(True, True, True, True))
    await call.message.delete()
    await call.message.answer("кт то купил размут ахаха")
...
async def work(message:Message, state:FSMContext):
   async with state.proxy() as data:
      if "me" not in data:
         data["me"] = datetime.now()
      else:
         if data["me"] + timedelta(seconds=30) > datetime.now():
            timer = (data["me"] + timedelta(seconds=180)) - datetime.now()
            await message.answer(f"Не так быстро! Подожди еще {timer.secouns} секунд")
            return 
         else:
            data["me"] = datetime.now()
   new_money = random.randint(0, 1000)
...
async def pingerd(message: Message,state:FSMContext):
    start = datetime.now()
    msg = await message.answer("Измеряю...")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    await sleep(0.2)
    await msg.edit_text('ping: '+str(ms))
